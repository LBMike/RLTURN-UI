...
interface SavingsDepositSheetProps {
  open: boolean;
  asset: "xrp" | "rlusd";
  onClose: () => void;
...
export function SavingsDepositSheet({ open, asset, onClose, onContinue }: SavingsDepositSheetProps) {
  const [amount, setAmount] = useState("");

  if (!open) return null;

  const isXrp = asset === "xrp";
  const { savings, xrp, rlusd } = mockData;
  ...
  const maxDeposit = isXrp
    ? parseFloat(xrp.available.replace(/,/g, ""))
    : parseFloat(rlusd.available.replace(/,/g, ""));
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative flex h-[680px] w-full max-w-md flex-col rounded-[16px] bg-white shadow-2xl">
        <div className="flex items-center justify-between px-6 pt-6">
          ...
          <h2 className="text-lg font-bold">Deposit {isXrp ? "XRP" : "RLUSD"}</h2>
          <button
            onClick={onClose}
            className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-[#f8fafc]"
          >
        ...
        <div className="flex-1 overflow-y-auto px-6 pt-8">
          <p className="text-sm text-[#475569]">
            How much {isXrp ? "XRP" : "RLUSD"} do you want to deposit into Savings?
          </p>
          ...
          <p className="mt-1 text-xs text-[#94a3b8]">
            Available: {isXrp ? `${xrp.available} XRP` : `${rlusd.available} RLUSD`}
          </p>
